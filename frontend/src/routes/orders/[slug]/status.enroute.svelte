<script>
	...
	const submit = async () => {
		...
		let result = await fetch(
			`${import.meta.env.VITE_BACKEND}/orders/${module.value.order.key}/status/enroute`,
			{
				method: 'put',
				headers: {
...
<Form title="Update Order Status" error={error.error}>
	<Note>
		Please give reason for updating the order status to <span class="bold">"enroute"</span>
	</Note>
